...
export default function UpdateUser() {
  ...
  const handleDisableEnableUser = (userId: string, active: boolean) => {
    ...
    try {
      request
        .patch(DISABLE_USER(userId), updatedUser, {
          headers: {
  ...
  const handleSaveName = async () => {
    ...
    const updatedGender =
      genderSelectRef.current?.value === "Male" ? true : false;
    ...
    if (
      ...
      try {
        await request
          .put(UPDATE_USER(userId as any), updatedUser, {
            headers: {
              "Content-Type": "application/json",
            },
          })
      ...
      console.log(
        `Updating user ${selectedUser.id} name to "${displayNameInputRef.current.value} + ' ' +${firstNameInputRef.current.value} + ' ' + ${lastNameInputRef.current.value}"`
      );
      ...
      <Dialog open={dialogEditOpen} onClose={handleCloseEditDialog}>
        ...
        <DialogContent>
          ...
          >
            ...
            <select
              id="gender"
              defaultValue={selectedUser?.gender ? "Male" : "Female"}
              ref={genderSelectRef}
              style={{ width: "55%" }}
            >
